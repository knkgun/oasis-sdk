...
func parsePolicy(net *config.Network, wallet *config.Wallet, policy string) *contracts.Policy {
	switch {
	case policy == "nobody":
	...
	case strings.HasPrefix(policy, "address:"):
		...
		address, err := config.ResolveAddress(net, policy)
		if err != nil {
...
func parseTokens(pt *config.ParaTime, tokens []string) []types.BaseUnits {
	...
	for _, raw := range tokens {
		...
		amount, err := common.ParseParaTimeDenomination(pt, raw, types.NativeDenomination)
		if err != nil {
