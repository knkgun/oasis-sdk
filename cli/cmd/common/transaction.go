...
func WaitForEvent(
	...
	pt *config.ParaTime,
	conn client.Connection,
	decoder sdkClient.EventDecoder,
	mapFn func(sdkClient.DecodedEvent) interface{},
) <-chan sdkClient.DecodedEvent {
	// Start watching events.
	ch, err := conn.Runtime(pt).WatchEvents(ctx, []sdkClient.EventDecoder{decoder}, false)
	cobra.CheckErr(err)

	// Start processing events.
	resultCh := make(chan sdkClient.DecodedEvent)
	go func() {
