...
use oasis_runtime_sdk::{
    ...
    storage::ConfidentialStoreError,
...
impl Module {
    ...
    fn get_key_pair<C: Context>(ctx: &mut C, id: &[u8]) -> Result<KeyPair, Error> {
        sdk::storage::confidential::get_key_pair(ctx, id).map_err(|err| match err {
            ConfidentialStoreError::KeyManagerUnavailable => {
                Error::Abort(sdk::dispatcher::Error::Aborted)
            }
            ConfidentialStoreError::KeyManagerFailure(err) => {
                Error::Abort(sdk::dispatcher::Error::KeyManagerFailure(err))
            }
        ...
        'a: 'b,
    {
        ...
        let confidential_store =
            sdk::storage::ConfidentialStore::new_with_key_pair(inner_store, key_pair);
